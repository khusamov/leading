...
Создать проект при помощи react-app-create
	...
	apply(compiler) {
		compiler.plugin('done', stats => {
			if (stats.compilation.errors && stats.compilation.errors.length > 0) {
				throw new Error('AfterWebpackPlugin', stats.compilation.errors);
			}
			this.actionFn();
		});
	}
            ...
            use: [
              {
				  ...
				  options: {
				    // disable type checker - we will use it in fork plugin
					  // нужны файлы declaration files поэтому поставил false
				    transpileOnly: false,
				...
				// {
				// 	loader: require.resolve('ts-loader-decleration')
				// }
            ],
          },
  ...
  plugins: [
    ...
    new ForkTsCheckerWebpackPlugin({
	  ...
	  // new DeclarationBundlerPlugin({
		//   moduleName:'eirc-w-react-packages',
		//   out:'bundle111.d.ts',
	  // })
