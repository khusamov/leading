...
Чтобы он не занимал лишнее место, нужно удалить каталог `ext`:

```bash
rmdir /S /Q ext
```

И прописать путь к фреймворку в файле `workspace.json`:
...
Как вариант, можно вместо прописывания ссылки в `workspace.json` заменить каталог `ext` симлинком на `c:/sencha/6.2.0`:
