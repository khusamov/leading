...
- [Быстрый старт Classic Toolkit, начало](#Быстрый-старт-Classic-Toolkit-начало)
...
Добавьте в раздел `scripts` строки запуска и сборки приложения:
...
```bash
cd c:/@repositories/myproject/mytestapp
sencha app watch
```
...
Команда [`sencha app watch`][sencha-app-watch] по сути создает веб-сервер, который показывает директорию рабочего пространства.
О чем в сообщениях команды сообщается. Зайдите в браузере по адресу http://localhost:1841 и найдите директорию с вашим приложением.
Когда кликлите по директории приложения, то откроется красивое приложение о сотрудниках компании.
Это тестовое приложение, которое по умолчанию автоматически генерируется командой `sencha generate app`.
